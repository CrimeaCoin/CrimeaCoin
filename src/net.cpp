...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "crimeacoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"crimeacoin.org", "dnsseed.crimeacoin.org"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Crimeacoin is probably already running."), addrBind.ToString().c_str());
        else
