...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // crimeacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "crimeacoin Signed Message:\n";
...
{
    // crimeacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 5100000); // Reward halves every 4 years
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // crimeacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // crimeacoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "11/Mar/2014 Crimea wants to start referendum on the subject of independence.";	
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // crimeacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("crimeacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("crimeacoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("crimeacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("crimeacoin-miner");
    ...
    try { loop {
        ...
        printf("Running crimeacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("crimeacoinMiner terminated\n");
        throw;
    }
}
